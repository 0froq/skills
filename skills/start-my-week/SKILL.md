...
4. **确认后落盘** - 周计划必须先完成 review 并获得用户明确 confirm，未 confirm 前不能写入正式文件
...
1. 先输出 review 版本（可读摘要 + 拟写入 YAML 预览）
2. 明确询问用户确认（例如："请确认是否按此版本写入正式文件"）
3. **仅当用户给出明确 confirm 后**，才能写入正式文件

未 confirm 时：
- ❌ 不得写入 `docs/dashboard/weekTasks/<weekId>.yml`
- ❌ 不得写入 `docs/dashboard/advisor/<weekId>-start.md`

confirm 后如果用户继续修改：
- 允许继续修改，但必须在对应任务的 `links` 中追加“变更记录”
- 变更记录仅写 `label`，不提供 `url`
- `label` 需包含：变更原因、时间、可选 AI 身份
