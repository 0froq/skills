...
function generateCommitMessage(weekId: string, status: StatusResult): string {
  ...
  // 检测变更类型
  const hasWeekReview = files.some(f => f.includes('weekTasks') && f.includes(weekId))
  const hasAdvisor = files.some(f => f.includes('advisor'))
  const hasBacklog = files.some(f => f.includes('monthBacklogs'))

  let action = 'update'
  if (hasWeekReview && !files.some(f => !f.includes(weekId) && !f.includes('advisor'))) {
    action = 'add'
  }
  ...
  if (action === 'add') {
    parts.push(`add week review for ${weekId}`)
  } else {
    parts.push(`update week review for ${weekId}`)
  }
...
function generateFileSummary(files: string[]): string {
  ...
  if (types.size === 1) return `${Array.from(types)[0]}-review`
...
function displayGitResult(result: GitOperationResult, weekId: string): void {
  ...
  if (result.success) {
    ...
    } else if (result.error?.type === 'COMMIT_FAILED' && result.error.message.includes('身份')) {
      console.log('\n💡 建议:')
...
async function executeGitWorkflow(
  ...
  if (!result.success && result.error?.type !== 'NOT_A_REPO') {
    ...
    console.log(`   git commit -m "docs(dashboard): update week review for ${weekId}"`)
...
```bash
# 新增复盘
docs(dashboard): add week review for 2026-03-31 (week-review)

# 更新复盘（包含多个文件）
docs(dashboard): update week review for 2026-03-31 (week,advisor)

# 更新复盘（包含 backlog 更新）
docs(dashboard): update week review for 2026-03-31 (week,advisor,backlog)
```
...
| 错误类型 | 常见原因 | 解决方案 |
|----------|----------|----------|
| NOT_A_REPO | 不在 git 仓库中 | 进入正确目录或初始化仓库 |
| NO_REMOTE | 未配置远程仓库 | 仅提交，跳过推送 |
| NO_CHANGES | 文件无变更 | 跳过提交 |
| COMMIT_FAILED | 未配置用户名/邮箱 | 配置 git 身份 |
| AUTH_FAILED | SSH/HTTPS 认证失败 | 检查密钥或凭证 |
| PUSH_FAILED | 远程有更新 | 先 pull 再 push |
