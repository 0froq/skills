...
description: 非破坏性写入策略 - 使用标记锚点更新 AI 生成的日计划内容
---
...
async function writeDayPlan(
  date: string,
  aiContent: string,
  dryRun: boolean = false
): Promise<WriteResult> {
  const filePath = `docs/dashboard/dayTodos/${date}.yml`
  ...
  if (!exists) {
    // 创建新文件
    finalContent = createNewFile(date, aiContent)
  } else {
    // 更新现有文件
...
function createNewFile(date: string, aiContent: string): string {
  const header = `# Day Plan: ${date}
...
async function writeWithUserApproval(
  ...
  // 3. 写入文件
  const result = await writeDayPlan(date, yamlContent, false)
...
async function createBackup(filePath: string): Promise<string> {
  const backupDir = 'docs/dashboard/backups/dayTodos'
...
```typescript
async function handleCarryOverTasks(
  today: string,
  yesterday: string,
  aiPlan: DayPlan
): Promise<DayPlan> {
  const yesterdayPath = `docs/dashboard/dayTodos/${yesterday}.yml`
  
  if (!await fileExists(yesterdayPath)) {
    return aiPlan
  }
  
  const yesterdayPlan = await readYaml(yesterdayPath)
  const incompleteTasks = (yesterdayPlan.tasks || [])
    .filter((t: Task) => t.status !== 'done')
    .map((t: Task) => ({
      ...t,
      carryOverFrom: yesterday,
      status: 'notStarted' // 重置为未开始
    }))
  
  // 将遗留任务插入到 AI 计划的前面
  const mergedTasks = [...incompleteTasks, ...aiPlan.tasks]
  
  // 限制总任务数
  const maxTasks = 8
  const limitedTasks = mergedTasks.slice(0, maxTasks)
  ...
  return {
    ...
    meta: {
      ...aiPlan.meta,
      carryOverCount: incompleteTasks.length
    }
  }
}
```
