...
async function gitWorkflow(
  ...
  // 5. 提交
  const commitResult = await commitChanges(git, date, statusAfterAdd)
...
async function commitChanges(
  ...
  try {
    const message = generateCommitMessage(date, status)
...
function generateCommitMessage(date: string, status: StatusResult): string {
  const files = status.staged.map(f => f.path)
  
  // 检测变更类型
  const hasNewDay = files.some(f => f.includes('dayTodos') && f.includes(date))
  const hasAdvisor = files.some(f => f.includes('advisor'))
  
  let action = 'update'
  if (hasNewDay && !files.some(f => !f.includes(date))) {
    action = 'add'
  }
  ...
  if (action === 'add') {
    parts.push(`add day plan for ${date}`)
  } else {
    parts.push(`update day plan for ${date}`)
  }
...
```typescript
function displayGitResult(result: GitOperationResult, date: string): void {
  console.log('')
  console.log('═══════════════════════════════════════')
...
async function executeGitWorkflow(
  ...
  const result = await gitWorkflow(date, writtenFiles)
  displayGitResult(result, date)
  ...
  if (!result.success && result.error?.type !== 'NOT_A_REPO') {
    ...
    console.log(`   git commit -m "docs(dashboard): add day plan for ${date}"`)
...
当一天内多次运行 start-my-day 时，可以使用 amend 合并提交：

```typescript
async function gitWorkflowWithAmend(
  date: string,
  files: string[],
  ...
  // 提交（使用 amend 如果指定）
  try {
    const message = generateCommitMessage(date, await git.status())
